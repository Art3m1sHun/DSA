...
int deletehashtable(Hashtable &, int);
...
if (ht.n >= (int)(LOAD*ht.M)) return 0;
...
Node *search(Hashtable ht, int maso){
    ...
    Node* curr = ht.table[index].head;
    while (curr){
        if (curr->data.Maso == maso) return curr;
        else curr = curr -> next;
    }
    return nullptr;
}

int deletehashtable(Hashtable ht, int maso){
	int index = Hash(ht, maso);
    Node* curr = ht.table[index].head;
    Node* prev = nullptr;
    while (curr){
        if (!prev&&curr->data.Maso==maso) {
